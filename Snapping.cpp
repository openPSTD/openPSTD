...
QVector2D Snapping::Snap(std::shared_ptr<Model> const &model, QVector2D vector)
{
    bool snap0 = false;
    bool snap1 = false;
    if(model->snapping->SnapToDomainEdges)
    {
        ...
        if(abs(min0 - vector[0]) < model->snapping->SnapToDomainEdgesDistance)
        {
            vector[0] = min0;
            snap0 = true;
        }
        if(abs(min1 - vector[1]) < model->snapping->SnapToDomainEdgesDistance)
        {
            vector[1] = min1;
            snap1 = true;
        }
