...
if [ ${TARGET} = "win64" ]; then
    ...
    if [ ${TRAVIS_OS_NAME} = "linux" ]; then
        ...
        cd fftw-3.3.5
        ./configure --enable-float --enable-sse --enable-sse2 --enable-avx --enable-avx2 --enable-generic-simd128 CFLAGS="-fPIC" CPPFLAGS="-fPIC" > /dev/null
        make > /dev/null
        sudo make install
    else
        ...
        tar -xzf fftw-3.3.5.tar.gz
        cd fftw-3.3.5
        ./configure --enable-float --enable-sse --enable-sse2 --enable-avx --enable-avx2 --enable-generic-simd128 CXX="g++ -stdlib=libstdc++ -arch x86_64 -fPIC" > /dev/null
        make > /dev/null
        sudo make install
    fi
fi
