...
//
//////////////////////////////////////////////////////////////////////////
/* comment about non staggered derivatives TODO*/
...
namespace OpenPSTD {
    namespace Kernel {
        ...
        WisdomCache::Discretization WisdomCache::discretize_wave_numbers(float dx, int N) {
            ...
            discr.pressure_deriv_factors_nonstag =  complex_wave_numbers;
            discr.velocity_deriv_factors_nonstag = complex_wave_numbers;
            discr.velocity_deriv_factors_strange = -complex_wave_numbers;
            discr.velocity_stag_factor_dy=(-complex_wave_numbers * (dx * 0.5)).exp() ;
            discr.velocity_stag_factor_dx=(complex_wave_numbers * (dx * 0.5)).exp();

            //discr.velocity_deriv_factors_nonderivating_ytox = -(dy * 0.5).exp() * complex_wave_numbers;

            return discr;
        }
// For the non derivative vy, define non derivative deriv factors, and also Lvalues and put them in the else if cases in calc.
