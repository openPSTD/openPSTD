...
namespace OpenPSTD {
    ...
    namespace Kernel {
       ...
       void Domain:: calcwind(std::shared_ptr<Domain> domain) {
           ...
           float bottomy = domain->bottom_right.y*domain->settings->GetGridSpacing();
            float rightx = domain->bottom_right.x*domain->settings->GetGridSpacing();
            float topy = domain->top_left.y*domain->settings->GetGridSpacing();
            float leftx = domain->top_left.x*domain->settings->GetGridSpacing();
          //cout<<domain->top_left.y<<endl;



            // cout << "The grid spacing is " << domain->settings->GetGridSpacing() << endl;
            //cout << "The size is " << (bottomy - topy) / domain->settings->GetGridSpacing() << endl;
            int gridy = (bottomy - topy) / domain->settings->GetGridSpacing();
            int gridx = (rightx - leftx) / domain->settings->GetGridSpacing();
            //cout<<endl<<topy<<" " << bottomy<<endl;
...
*/
        ...
        /*void Domain:: calcwind(std::shared_ptr<Domain> domain) {
          ...
          //cout<<domain->top_left.y<<endl;
            ...
            Eigen::ArrayXf  z(gridy), z_stag(gridy), u(gridy), du(gridy), u_stag(gridy), du_stag(gridy),x(gridy), x_stag(gridy);


            Eigen :: ArrayXXf umat(gridy,gridx), umat_stag(gridy,gridx), dumat(gridy,gridx), dumat_stag(gridy,gridx);


            float k = 0.4, ustar = 0.3;
            float Bo = 0.015, L;
           ...
           for (int i = 0; i <gridy; i++) {
            ...
            for (int i = 0; i < gridy; ++i) {
                for (int j = 0; j < gridx; j++) {
                    umat(i,j) = u(i);
                    umat_stag(i,j)=u_stag(i);
                    dumat(i,j) = du(i);
                    dumat_stag(i,j)=du_stag(i);
...
*/
        ...
        ArrayXXf Domain::calc(CalcDirection cd, CalculationType ct,  ArrayXcf dest) {
           ...
           /* cout<<coun<<endl;
            coun++;*/
            /*if (ct==CalculationType::VELOCITY_dvydx)
                cout<<"VELOCITY_dvydx"<<endl;
            else if (ct==CalculationType:+:VELOCITY_nonstag)
                cout<<"VELOCITY_nonstag"<<endl;
            else if (ct==CalculationType::VELOCITY_stagdown)
                cout<<"VELOCITY_stagdown"<<endl;
            else if (ct==CalculationType::VELOCITY_stagright)
                cout<<"VELOCITY_stagright"<<endl;
            else if (ct==CalculationType::VELOCITY)
                cout<<"VELOCITY"<<endl;
            else if (ct==CalculationType::PRESSURE)
                cout<<"PRESSURE"<<endl;
            else if (ct==CalculationType::PRESSURE_nonstag)
                cout<<"PRESSURE_nonstag"<<endl;*/
            ...
            if (dest.rows() != 0) {
                ...
                // Additional L value for Lvxvy TODO Sai
          /*      if (ct == CalculationType::PRESSURE) {*/
                    if (ct == CalculationType::PRESSURE||ct == CalculationType::PRESSURE_nonstag) {
                    if (cd == CalcDirection::X) {
                    ...
                    if (cd == CalcDirection::X && ct==CalculationType::VELOCITY) {
                        source = this->l_values.Lvx;
                    }
                        else if (cd == CalcDirection::X && ct== CalculationType::VELOCITY_nonstag)
                    {
                        source = this->l_values.Lvx_nonstag;
                    }
                        else if (cd== CalcDirection ::Y && ct== CalculationType::VELOCITY)
                    {
                        source = this->l_values.Lvy;
                    }
                        else if
                            (cd== CalcDirection ::Y && ct== CalculationType::VELOCITY_nonstag)
                    {
                        source = this->l_values.Lvy_nonstag;
                    }
                    else if(cd== CalcDirection ::X && ct== CalculationType::VELOCITY_dvydx)
                    {
                        source = this->l_values.Lvydx;
                    }
                        else if (cd==CalcDirection::Y && ct== CalculationType::VELOCITY_stagdown)
                    {
                        source= this->l_values.Lvy_ystag;
                    }
                        else if(cd== CalcDirection::X && ct==CalculationType::VELOCITY_stagright)
                        {
                            source= this->l_values.Lvy_xstag;
            ...
            for (int i = 0; i != domains1.size() + 1; i++) { //the +1 because a null pointer is also needed
                ...
                for (int j = 0; j != domains2.size() + 1; j++) {
                   ...
                   if (ct == CalculationType::PRESSURE ||ct==CalculationType::VELOCITY_stagdown) {
                    ...
                    else if (ct == CalculationType::PRESSURE_nonstag|| ct== CalculationType::VELOCITY_dvydx) {
// No need to add anything
                    ...
                    // If statement delete and if it's velocity, extend zero

                    if (ct == CalculationType::VELOCITY|| ct== CalculationType ::VELOCITY_nonstag||ct==CalculationType ::VELOCITY_dvydx || ct==CalculationType::VELOCITY_stagdown||ct==CalculationType::VELOCITY_stagright) {
                        if (d1 == nullptr ) {
                            d1 = shared_from_this();
                            if (cd == CalcDirection::X) {
...
// Replaced by the above fragment of code because the code below seems incorrect for arbitrary configurations
...
// Remove non staggered part of current values because they dont exist TODO sai
                    if (ct == CalculationType::PRESSURE) {
                    ...
                    else if (cd == CalcDirection::X && ct == CalculationType::VELOCITY) {
                    ...
                    else if (cd == CalcDirection::Y && ct == CalculationType::VELOCITY_nonstag){
                        matrix_main = current_values.vy0;
                    }
                        else if (ct == CalculationType::VELOCITY_dvydx )
                    ...
                    if (dest.rows() != 0) {
                        ...
                        {
                            derfact = wnd->get_discretization(settings->GetGridSpacing(),N_total).pressure_deriv_factors_nonstag;
                        }
                            else if (ct == CalculationType::VELOCITY)
                        { derfact = wnd->get_discretization(settings->GetGridSpacing(),
                            ...
                            derfact = wnd->get_discretization(settings->GetGridSpacing(),
                                                              N_total).velocity_deriv_factors_nonstag;
                        }

                        else if (ct==CalculationType::VELOCITY_dvydx){

                            derfact = wnd->get_discretization(settings->GetGridSpacing(),
                                                              N_total).velocity_deriv_factors_strange;

                        }
                        else if (ct==CalculationType::VELOCITY_stagdown){

                            derfact = wnd->get_discretization(settings->GetGridSpacing(),
                                                              N_total).velocity_stag_factor_dy;

                        }
                        else if (ct==CalculationType::VELOCITY_stagright){

                            derfact = wnd->get_discretization(settings->GetGridSpacing(),
                                                              N_total).velocity_stag_factor_dx;


                        }
                    }// Add new velocity vydx TODO Sai
                    ...
                    if (cd == CalcDirection::X) {
                        ...
                        if (ct==CalculationType::VELOCITY_stagdown){
            ...
            if (dest.rows() == 0) {
                ...
                {if (cd == CalcDirection::X) {
                        ...
                        this->l_values.Lpy_nonstag = source;
                    }
                ...
                {if (cd == CalcDirection::X) {
                        this->l_values.Lvx_nonstag = source;
                    }
                    else {
                        this->l_values.Lvy_nonstag = source;
                    }
                }
                else if (ct==CalculationType::VELOCITY_dvydx)
                {
                    this->l_values.Lvydx=source;
                }

                else if (ct==CalculationType::VELOCITY_stagdown)
                {
                    this->l_values.Lvy_ystag=source;
                }
        ...
        void Domain::calc(CalcDirection cd, CalculationType ct) {
            ...
            if (ct == CalculationType::VELOCITY_stagdown && cd == CalcDirection::X)
            {
                return;
            }
       ...
       void Domain::clear_matrices() {
           ...
           /* l_values.Lpx_nonstag=extended_zeros(0,0);
            l_values.Lpy_nonstag=extended_zeros(0,0);
            l_values.Lvx_nonstag=extended_zeros(0,1);
            l_values.Lvy_nonstag=extended_zeros(1,0);
            l_values.Lvy_ystag=extended_zeros(1,0);
            l_values.Lvy_xstag=extended_zeros(0,0);
            l_values.Lvydx= extended_zeros(0,0);*/
        }
      ...
      /*      ArrayXXf alpha_pml_velocity =
            ...
            ArrayXXf alpha_pml_velocity =
                     settings->GetAttenuationOfPMLCells() * velocity_range.pow(4);
            /*ArrayXXf pressure_pml_factors = (-alpha_pml_pressure * settings->GetTimeStep() /
                                             settings->GetDensityOfAir()).exp();*/

            ArrayXXf pressure_pml_factors = (-alpha_pml_pressure * settings->GetTimeStep()).exp();
