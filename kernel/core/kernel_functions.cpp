...
namespace OpenPSTD {
    namespace Kernel {
        RhoArray get_rho_array(const float rho1, const float rho_self, const float rho2) {
           ...
           /* result.pressure_nonstag = ArrayXXf(4, 2);
            result.velocity_nonstag = ArrayXXf(4, 2);;
            result.pressure_nonstag << rlw1, rlw2,
                    rrw1, rrw2,
                    tlw1, tlw2,
                    trw1, trw2;

            result.velocity_nonstag << -rlw1, -rlw2,
                    -rrw1, -rrw2,
                    tlw2, tlw1,
                    trw2, trw1;


            result.velocity_dvydx = ArrayXXf(4, 2);;

            result.velocity_dvydx << -rlw1, -rlw2,
                    -rrw1, -rrw2,
                    tlw2, tlw1,
                    trw2, trw1;

            result.velocity_stagdown = ArrayXXf(4,2);

            result.velocity_stagdown << -rlw1, -rlw2,
                    -rrw1, -rrw2,
                    tlw2, tlw1,
                    trw2, trw1;

            result.velocity_stagright = ArrayXXf(4,2);

            result.velocity_stagright<<  -rlw1, -rlw2,
                    -rrw1, -rrw2,
                    tlw2, tlw1,
                    trw2, trw1;
*/
...
if (ct==CalculationType::PRESSURE){
  ...
  result = derived_array.leftCols(wlen + p2.cols() ).rightCols(p2.cols() );
        ...
        void write_array_to_file(Eigen::ArrayXXf array, std::string filename, unsigned long kk, int id) {
            ...
            std::string fullname = "cpp_"+std::to_string(kk/6)+"_domain_"+std::to_string(id)+".csv";
            std::ofstream data_stream(data_folder + "/" + fullname);
           //data_stream <<  filename << "_" << kk + 1 ;
