...
void Simulator::start() {
	...
	threadID = pthread_create(&thread, nullptr, &Simulator::runstatic, this);
	threadRunning = true;
}
...
void Simulator::draw(QImage* pixels) {
	...
	for (unsigned int i = 0; i < model->domains.size(); i++) {
		...
		unsigned int fwidth = frames[0].getWidth(i);
		...
		for (int y = miny; y < maxy; y++) {
			for (int x = minx; x < maxx; x++) {
				...
				double xk = static_cast<double>(x-minx) * (fwidth-1) / (maxx-minx-3);
				double yk = static_cast<double>(maxy-y) * (fheight-1) / (maxy-miny-3);
				unsigned int xk0 = static_cast<unsigned int>(xk);
				unsigned int xk1 = xk0 + 1;
				unsigned int yk0 = static_cast<unsigned int>(yk);
				unsigned int yk1 = yk0 + 1;
				double tx = xk - xk0;
				double ty = yk - yk0;
