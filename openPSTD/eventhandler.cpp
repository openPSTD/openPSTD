...
void EventHandler::mousePress(
	...
	std::vector<unsigned int> selectedDomains;
...
void EventHandler::mouseDrag(int x, int y, bool drag, Qt::KeyboardModifiers modifiers) {
	...
	if (model->state == SELECT && drag && modifiers == Qt::NoModifier) {
		...
		for (unsigned int i = 0; i < selectedWalls.size(); i++) {
			...
			unsigned int domainID = selectedWalls[i].first;
			unsigned int wallID = selectedWalls[i].second;
...
void EventHandler::drawMeasure(QImage* pixels) {
	...
	for (int t = 0; t < 1000; t++) {
		double tt = static_cast<double>(t) / 1000;
		
		int x = static_cast<int>(tt * x0 + (1 - tt) * x1);
		int y = static_cast<int>(tt * y0 + (1 - tt) * y1);
...
void EventHandler::deleteSelected() {
	...
	for (unsigned int i = 0; i < selectedSources.size(); i++) {
		// Delete this source
		unsigned int sourceID = selectedSources[i];
	...
	for (unsigned int i = 0; i < selectedReceivers.size(); i++) {
		...
		unsigned int receiverID = selectedReceivers[i];
...
void EventHandler::moveToCenter() {
	...
	int dx = maxx - minx;
	int dy = maxy - miny;
	int maxzoomx = static_cast<int>(w / (1.1 * dx));
	int maxzoomy = static_cast<int>(h / (1.1 * dy));
