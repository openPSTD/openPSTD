...
void validate(boost::any& v, const std::vector<std::string>& values, CLIDomainAdd*, int)
{
    ...
    if (regex_match(s, match, r)) {
        v = any(CLIDomainAdd(lexical_cast<int>(match[1]), lexical_cast<float>(match[2]), lexical_cast<float>(match[3]),
...
void validate(boost::any& v, const std::vector<std::string>& values, CLIDomainChange*, int)
{
    ...
    smatch match;
    if (regex_match(s, match, r)) {
        v = any(CLIDomainChange(lexical_cast<int>(match[1]), lexical_cast<float>(match[2]), lexical_cast<float>(match[3]),
